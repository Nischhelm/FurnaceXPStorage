...
public abstract class SmeltingRecipeMixin {
    ...
    private float furnaceXpStorage_jeiSmeltingRecipe_drawXPInfo(FurnaceRecipes instance, ItemStack stack, Operation<Float> original){
        ...
        //inputs.get(1) can be bigger than size 1, if there is multiple metadata items that all smelt to the same output, registered via the wildcard metadata 32767
        ...
        return !inputs.isEmpty() && !inputs.get(0).isEmpty() ? SmeltingExperiencesByInput.getSmeltingExperience(inputs.get(0).get(0),stack) : original.call(instance, stack);
    }
}
